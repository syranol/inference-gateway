...
def test_settings_defaults(monkeypatch):
    ...
    monkeypatch.delenv("UPSTREAM_MAX_RETRIES", raising=False)
    monkeypatch.delenv("UPSTREAM_RETRY_BACKOFF", raising=False)
    ...
    assert settings.upstream_max_retries == 3
    assert settings.upstream_retry_backoff == 1.0
...
def test_settings_overrides(monkeypatch):
    ...
    monkeypatch.setenv("UPSTREAM_RETRY_BACKOFF", "0.5")
    ...
    assert settings.upstream_max_retries == 5
    assert settings.upstream_retry_backoff == 0.5
